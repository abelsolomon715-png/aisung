...
app.post('/api/generate-track', async (req, res) => {
    try {
        ...
        // Exact formatting matching Apiframe's current Suno generation expectations
        const payloadBody = {
            model: "suno",
            custom: true,
            prompt: isInstrumental ? "[Instrumental Track]" : "[Verse 1]\nGenerated by AISUNG automated orchestration loop.",
            title: `${subGenre} Creation`,
            sunoParams: {
                model_version: "V4",
                style: refinedPrompt,
                instrumental: isInstrumental ? true : false
            }
        };
